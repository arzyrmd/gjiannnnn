...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-slate-900 border-2 border-emerald-500/50 rounded-2xl p-5 md:p-6 shadow-xl relative overflow-hidden flex flex-col justify-between space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="text-xs font-mono-num font-bold text-slate-400 bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-800">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end pt-1">
                <div class="sm:col-span-7">
                    ...
                    <div id="metricPendapatanHariIni" class="text-2xl sm:text-3xl md:text-4xl font-black font-mono-num text-emerald-400 tracking-tight mt-1 transition-all">
                        Rp {{ number_format($pendapatanHariIni, 0, ',', '.') }}
                    </div>
                </div>
                <div class="sm:col-span-5 text-left sm:text-right sm:border-l border-slate-800 sm:pl-4 space-y-1">
                    <div>
        ...
        <div class="bg-slate-900 border-2 border-cyan-500/50 rounded-2xl p-5 md:p-6 shadow-xl relative overflow-hidden flex flex-col justify-between space-y-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end pt-1">
                <div class="sm:col-span-7">
                    <span class="text-[11px] font-extrabold uppercase tracking-wider text-slate-400 block">Total Pendapatan</span>
                    <div id="metricPendapatanBulanIni" class="text-2xl sm:text-3xl md:text-4xl font-black font-mono-num text-cyan-400 tracking-tight mt-1 transition-all">
    ...
    <div class="bg-slate-900 border-2 border-amber-400/90 rounded-2xl p-5 md:p-6 shadow-2xl space-y-5">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
                <h2 class="text-base md:text-lg font-black uppercase tracking-wider text-white">
                    Catat Job Order Baru
                </h2>
                <p class="text-xs text-slate-400 mt-0.5">Form input cepat pekerjaan atau piket harian teknisi.</p>
            </div>
            <span class="px-3 py-1 rounded-md bg-amber-400/10 border border-amber-400/30 text-amber-400 text-[11px] font-extrabold uppercase tracking-wider">
                Input Cepat
            </span>
        </div>
        ...
        <form action="{{ route('job-orders.store') }}" method="POST" class="space-y-4" id="quickJobForm" onsubmit="handleQuickJobFormSubmit(event)">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                ...
                <div class="space-y-1.5 md:col-span-5">
                    ...
                    <select 
                        ...
                        id="tarif_id" 
                        required 
                        class="w-full px-3.5 py-3 rounded-xl bg-slate-950 border-2 border-slate-700 text-white font-bold focus:border-amber-400 focus:outline-none transition-colors text-sm"
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <label class="cursor-pointer">
                            <input 
                                ...
                                onchange="updatePricePreview()"
                            >
                            <div class="w-full py-3 px-3 rounded-xl bg-slate-950 border-2 border-slate-800 peer-checked:border-emerald-500 peer-checked:bg-emerald-950/80 peer-checked:text-emerald-300 text-slate-400 font-black text-center text-xs uppercase tracking-wider transition-all shadow-sm">
                                BERHASIL
                            </div>
                        </label>
                        ...
                        <label class="cursor-pointer">
                            <input 
                                ...
                                onchange="updatePricePreview()"
                            >
                            <div class="w-full py-3 px-3 rounded-xl bg-slate-950 border-2 border-slate-800 peer-checked:border-rose-500 peer-checked:bg-rose-950/80 peer-checked:text-rose-300 text-slate-400 font-black text-center text-xs uppercase tracking-wider transition-all shadow-sm">
                                GAGAL
                            </div>
                        </label>
                    </div>
                ...
                <div class="space-y-1.5 md:col-span-3">
                    <label for="tanggal" class="block text-xs font-extrabold text-slate-300 uppercase tracking-wider">
                        Tanggal Pekerjaan <span class="text-amber-400">*</span>
                    </label>
                    <input 
                        type="date" 
                        name="tanggal" 
                        ...
                        required 
                        class="w-full px-3.5 py-3 rounded-xl bg-slate-950 border-2 border-slate-700 text-white font-mono font-bold focus:border-amber-400 focus:outline-none transition-colors text-sm"
                    >
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center pt-1">
                <div class="sm:col-span-8">
                    <input 
                        ...
                        value="{{ old('catatan') }}"
                        class="w-full px-3.5 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 text-xs focus:border-amber-400 focus:outline-none"
                    >
                </div>
                ...
                <div id="customTarifContainer" class="sm:col-span-4 hidden">
                    <input 
                        ...
                        class="w-full px-3.5 py-3 rounded-xl bg-slate-950 border-2 border-amber-400 text-amber-400 font-mono-num font-bold text-xs focus:outline-none"
                    >
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 border-t border-slate-800/80">
                ...
                <button 
                    type="submit" 
                    class="w-full sm:w-auto py-3.5 px-8 rounded-xl bg-amber-400 hover:bg-amber-300 active:scale-[0.99] text-slate-950 font-black text-sm uppercase tracking-wider shadow-lg shadow-amber-400/20 transition-all cursor-pointer border-2 border-amber-300"
                >
                    SIMPAN JOB ORDER
                </button>
            </div>
    ...
    <div class="bg-slate-900 border-2 border-slate-800 rounded-2xl p-5 md:p-6 shadow-xl space-y-5">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
                <h3 class="text-base md:text-lg font-black uppercase text-white tracking-wider flex items-center gap-2">
                    ...
                    <span class="text-amber-400">({{ \Carbon\Carbon::createFromDate($year, $month, 1)->translatedFormat('F Y') }})</span>
                </h3>
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                <a 
                    ...
                    class="flex-1 md:flex-initial px-3.5 py-2.5 rounded-xl bg-emerald-950 border-2 border-emerald-500/80 hover:bg-emerald-900 text-emerald-300 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors"
                >
                ...
                <a 
                    ...
                    target="_blank"
                    class="flex-1 md:flex-initial px-3.5 py-2.5 rounded-xl bg-cyan-950 border-2 border-cyan-500/80 hover:bg-cyan-900 text-cyan-300 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors"
                >
        ...
        <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-3.5 space-y-3">
            <div class="flex items-center gap-2 border-b border-slate-800/80 pb-2">
                <button 
                    ...
                    id="tabBulan"
                    class="px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all cursor-pointer {{ (!$startDate && !$endDate) ? 'bg-amber-400 text-slate-950 font-black shadow' : 'bg-slate-800 text-slate-400 hover:bg-slate-700' }}"
                >
                    Filter Per Bulan
                </button>
                <button 
                ...
                @if($startDate || $endDate || $selectedBulan !== \Carbon\Carbon::now()->format('Y-m'))
                    <a href="{{ route('dashboard') }}" class="ml-auto text-xs text-rose-400 hover:underline font-bold uppercase tracking-wider">
                        Reset Filter
                    </a>
                @endif
            </div>
                ...
                <div class="flex flex-col sm:flex-row items-end gap-3">
                    <div class="w-full sm:w-64 space-y-1">
                        ...
                        <input 
                            ...
                            class="w-full px-3.5 py-2 rounded-xl bg-slate-900 border-2 border-slate-700 text-white font-mono font-bold text-xs focus:border-amber-400 focus:outline-none"
                        >
                    </div>
                    <button type="submit" class="w-full sm:w-auto px-5 py-2 rounded-xl bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider hover:bg-amber-300 transition-colors border-2 border-amber-300">
                        Tampilkan Rekap
                    </button>
                </div>
            </form>
            ...
            <form id="formFilterRentang" method="GET" action="{{ route('dashboard') }}" class="{{ ($startDate || $endDate) ? 'block' : 'hidden' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                    ...
                    <div>
                        <button 
                            type="submit" 
                            class="w-full py-2 px-5 rounded-xl bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider hover:bg-amber-300 transition-colors border-2 border-amber-300"
                        >
                            Terapkan Rentang
                        </button>
                    </div>
            ...
            <div>
                <h3 class="text-base font-black uppercase text-white tracking-wider flex items-center gap-2">
                    ...
                    @if($startDate && $endDate)
                        <span class="text-xs bg-amber-400/10 border border-amber-400/40 text-amber-400 px-2.5 py-0.5 rounded font-bold">
                            Rentang: {{ \Carbon\Carbon::parse($startDate)->format('d/m/Y') }} s/d {{ \Carbon\Carbon::parse($endDate)->format('d/m/Y') }}
                        </span>
                    @endif
                </h3>
                <p class="text-xs text-slate-400">Rincian entri per kategori tugas beserta status dan nilai tarif snapshot.</p>
            </div>
        </div>
                            ...
                            <td class="py-3.5 px-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1.5">
                                    <button 
                                        ...
                                        class="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-amber-400 border border-slate-700 font-bold text-[11px] uppercase tracking-wider transition-colors flex items-center gap-1"
                                    >
                                    ...
                                    <form action="{{ route('job-orders.destroy', $job->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus job order ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-2.5 py-1 rounded bg-rose-950/80 hover:bg-rose-900 text-rose-300 border border-rose-700/60 font-bold text-[11px] uppercase tracking-wider transition-colors flex items-center gap-1">
...
<div id="editModal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 hidden">
    <div class="bg-slate-900 border-2 border-amber-400 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl relative">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <button onclick="closeEditModal()" class="text-slate-400 hover:text-white font-bold text-xs uppercase tracking-wider">
                Tutup
            </button>
        </div>
        ...
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <select name="status" id="edit_status" required class="w-full px-3 py-2.5 rounded-xl bg-slate-950 border-2 border-slate-700 text-white font-bold text-xs focus:border-amber-400 focus:outline-none">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="tanggal" id="edit_tanggal" required class="w-full px-3 py-2.5 rounded-xl bg-slate-950 border-2 border-slate-700 text-white font-mono font-bold text-xs focus:border-amber-400 focus:outline-none" />
                </div>
            </div>

            <div class="space-y-1.5">
                <label for="edit_tarif" class="block text-xs font-extrabold text-slate-300 uppercase tracking-wider">Nominal Tarif (Rp)</label>
                <input type="number" name="tarif" id="edit_tarif" required min="0" step="500" class="w-full px-3 py-2.5 rounded-xl bg-slate-950 border-2 border-slate-700 text-white font-mono font-bold text-xs focus:border-amber-400 focus:outline-none" />
            </div>

            <div class="space-y-1.5">
                <label for="edit_catatan" class="block text-xs font-extrabold text-slate-300 uppercase tracking-wider">Catatan</label>
                <input type="text" name="catatan" id="edit_catatan" class="w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-white text-xs focus:border-amber-400 focus:outline-none" />
            </div>
...
<script>
    function switchFilterMode(mode) {
        ...
        if (mode === 'bulan') {
            ...
            tabBulan.className = "px-3.5 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider bg-amber-400 text-slate-950 shadow";
            tabRentang.className = "px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider bg-slate-800 text-slate-400 hover:bg-slate-700";
        } else {
            ...
            tabRentang.className = "px-3.5 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider bg-amber-400 text-slate-950 shadow";
            tabBulan.className = "px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider bg-slate-800 text-slate-400 hover:bg-slate-700";
        }
    }
